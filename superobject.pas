...
{$IF defined(VER230) or defined(VER240)  or defined(VER250) or
     defined(VER260) or defined(VER270)  or defined(VER280)or defined(VER300)}
{$DEFINE VER210ORGREATER}
...
{$ENDIF}
  ...
  TSuperObjectArray = array [0 .. (high(integer) div sizeof(TSuperObject)) - 1]
    of ISuperObject;
...
{$IFDEF HAVE_RTTI}
  ...
  TSuperRttiContext = class
  private
    class function GetAttrName(r: TRttiMember): string;
    class function GetFieldDefault(r: TRttiMember; const Obj: ISuperObject)
      : ISuperObject;
    class function IsNotAttrIgnored(r: TRttiMember): boolean;
  public
    ...
    function ToJson(var Value: TValue; const index: ISuperObject)
  ...
  public
    ...
    constructor FromJson(const Obj: ISuperObject;
      ctx: TSuperRttiContext = nil); overload;
    constructor FromJson(const str: string;
      ctx: TSuperRttiContext = nil); overload;
  end;
{$ENDIF}
...
begin
  ...
  if {$IF defined(NEED_FORMATSETTINGS)}FormatSettings.{$IFEND}DecimalSeparator
    <> '.' then
  begin
    p := PSOChar(Result);
    while p^ <> #0 do
      if p^ <> SOChar({$IF defined(NEED_FORMATSETTINGS)}FormatSettings.{$IFEND}DecimalSeparator)
      then
        Inc(p)
      else
      begin
...
type
  ...
  function ishex(const C: SOChar): boolean; {$IFDEF HAVE_INLINE} inline;
{$ENDIF}
  begin
    Result := (C < #256) and (AnsiChar(C) in ['0' .. '9', 'a' .. 'z',
      'A' .. 'Z'])
  end;
...
function TSuperObject.QueryInterface
...
{$IFDEF HAVE_RTTI}
  ...
  class function TSuperRttiContext.GetAttrName(r: TRttiMember): string;
  var
    O: TCustomAttribute;
  begin
  ...
  class function TSuperRttiContext.IsNotAttrIgnored(r: TRttiMember): boolean;
  var
    O: TCustomAttribute;
  begin
    Result := true;
  ...
  class function TSuperRttiContext.GetFieldDefault(r: TRttiMember;
            ...
            for F in Context.GetType(Value.AsObject.ClassType).GetFields do
              ...
              begin
                ...
                Result := FromJson(F.FieldType.Handle,
                  GetFieldDefault(F, Obj.AsObject[GetAttrName(F)]), v);
                if Result then
            ...
            for p in Context.GetType(Value.AsObject.ClassType).GetProperties do
              ...
              begin
                ...
                Result := FromJson(p.PropertyType.Handle,
                  GetFieldDefault(p, Obj.AsObject[GetAttrName(p)]), v);
                if Result then
  ...
  function TSuperRttiContext.ToJson(var Value: TValue;
        ...
        begin
          ...
          for F in Context.GetType(Value.AsObject.ClassType).GetFields do
            if (F.FieldType <> nil) and
              (F.Visibility = tmembervisibility.mvPublic) and
              (IsNotAttrIgnored(F)) then
            begin
              v := F.GetValue(Value.AsObject);
              Result.AsObject[GetAttrName(F)] := ToJson(v, index);
            end;

          for p in Context.GetType(Value.AsObject.ClassType).GetProperties do
            if (p.PropertyType <> nil) and
              (p.Visibility = tmembervisibility.mvPublic) and
              (IsNotAttrIgnored(p)) then
            begin
              v := p.GetValue(Value.AsObject);
              Result.AsObject[GetAttrName(p)] := ToJson(v, index);
            end;
    ...
    begin
      ...
      for F in Context.GetType(Value.TypeInfo).GetFields do
        if (F.Visibility = tmembervisibility.mvPublic) then
...
{$ENDIF}
    ...
    begin
      if TValueData(Value).FAsClass <> nil then
        Result := TSuperObject.Create
          (string(TValueData(Value).FAsClass.UnitName + '.' + TValueData(Value)
          .FAsClass.ClassName))
      else
        Result := nil;
    end;
